...
import (
	...
	"github.com/milena-mognon/fc-ports-and-adapters/adapters/cli"
...
func TestRu(t *testing.T) {
	...
	resultExpected := fmt.Sprintf("Product ID %s with the name %s has been created with the price %f and the status %s",
		productId,
		productName,
		productPrice,
		productStatus)
